...
def node_arrow():
     ...
     for count in range(numero_archi):
          ...
          #      archi_grafo_arrow.append([edge_list[count].attributes['source'].value, edge_list[count].attributes['target'].value, 'curve', weight, arrow_list[count].attributes['type'].value])
          # else:
          if arrow_list[count].attributes['target'].value == 'none' and arrow_list[count].attributes['source'].value == 'none':
               archi_grafo_arrow.append([edge_list[count].attributes['source'].value, edge_list[count].attributes['target'].value, 'line', weight, arrow_list[count].attributes['type'].value])

          elif arrow_list[count].attributes['target'].value == 'standard' and arrow_list[count].attributes['source'].value == 'none':
               archi_grafo_arrow.append([edge_list[count].attributes['source'].value, edge_list[count].attributes['target'].value, 'arrow', weight, arrow_list[count].attributes['type'].value])

          elif arrow_list[count].attributes['target'].value == 'standard' and arrow_list[count].attributes['source'].value == 'standard':
               archi_grafo_arrow.append([edge_list[count].attributes['source'].value, edge_list[count].attributes['target'].value, 'bidirectional', weight, arrow_list[count].attributes['type'].value])
     return archi_grafo_arrow

def edge_string():
     edge_to_string = '['
     for count in range(numero_archi):
          edge_to_string += f'({edge_list[count].attributes["source"].value},{edge_list[count].attributes["target"].value})'
     return f'{edge_to_string}]'
